...
export function ShareButtons({ url, title }: { url: string; title: string }) {
  ...
  const redditHref = `https://www.reddit.com/submit?url=${enc(url)}&title=${enc(title)}`;
  const whatsappHref = `https://wa.me/?text=${enc(`${title} (via Optimixed) ${url}`)}`;
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 border-t border-outline-variant pt-4">
      ...
      <a href={redditHref} target="_blank" rel="noreferrer" aria-label="Share on Reddit" title="Share on Reddit" className={btn}>
        <Brand path={REDDIT_PATH} label="Reddit" />
      </a>
      <a href={whatsappHref} target="_blank" rel="noreferrer" aria-label="Share on WhatsApp" title="Share on WhatsApp" className={btn}>
        <Brand path={WA_PATH} label="WhatsApp" />
      </a>
